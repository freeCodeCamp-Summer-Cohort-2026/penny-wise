...
const inputClass =
  'w-full rounded-lg border border-[var(--border)] bg-[var(--code-bg)] px-3 py-2.5 text-[15px] text-[var(--text-h)] transition-colors outline-none placeholder:text-[var(--text)] placeholder:opacity-70 focus:border-[var(--accent-border)] focus:ring-2 focus:ring-[var(--accent-border)] disabled:cursor-not-allowed disabled:opacity-70';
    ...
    return (
      <main className='mx-auto w-full max-w-4xl p-4 text-left sm:p-6'>
        <nav aria-label='Breadcrumb' className='text-sm text-[var(--text)]'>
          ...
          <span className='font-medium text-[var(--text-h)]'>My Profile</span>
        </nav>
        <h1 className='mt-2 text-3xl font-semibold tracking-tight text-[var(--text-h)]'>
          My Account
        </h1>
        <section className='mt-6 rounded-2xl border border-[var(--border)] bg-[var(--bg)] p-8 text-center shadow-[var(--shadow)]'>
  ...
  return (
    <main className='mx-auto w-full max-w-6xl p-4 text-left sm:p-6'>
      ...
      <div className='mt-6 grid gap-6 lg:grid-cols-[280px_1fr]'>
        ...
        <aside className='h-fit rounded-2xl border border-[var(--border)] bg-[var(--bg)] p-6 text-center shadow-[var(--shadow)]'>
          {user.avatar ? (
            <img
              src={user.avatar}
              alt={`${user.displayName}'s avatar`}
              className='mx-auto h-24 w-24 rounded-full border border-[var(--border)] object-cover'
            />
          ) : (
            <CircleUser
              aria-hidden='true'
              className='mx-auto h-24 w-24 text-[var(--text)]'
            />
          )}
          <h2 className='mt-4 text-xl font-semibold text-[var(--text-h)]'>
            {user.displayName}
          </h2>
          <p className='mt-1 text-sm break-all text-[var(--text)]'>
            {user.email}
          </p>
          {user.role && (
            <p className='mt-3 inline-block rounded-full bg-[var(--accent-bg)] px-3 py-1 text-xs font-semibold text-[var(--accent-bold)] capitalize dark:text-[var(--accent)]'>
              {user.role}
            </p>
          )}
          <div className='mt-4 rounded-xl bg-[var(--accent-bg)] p-4'>
            <p className='text-sm font-bold tracking-wide text-[var(--text-h)]'>
              LEVEL {level}
            </p>
          ...
          {stats.length > 0 && (
            <dl className='mt-4 space-y-2 border-t border-[var(--border)] pt-4 text-left text-sm'>
              {stats.map((row) => (
                <div key={row.label} className='flex justify-between gap-3'>
                  <dt className='text-[var(--text)]'>{row.label}</dt>
        ...
        <section
          ...
          className='rounded-2xl border border-[var(--border)] bg-[var(--bg)] p-6 shadow-[var(--shadow)] sm:p-8'
        >
          <h2
            id='personal-info-heading'
            className='border-b border-[var(--border)] pb-4 text-lg font-semibold text-[var(--text-h)]'
          >
          ...
          <div className='mt-6 flex flex-wrap items-center gap-4'>
            {user.avatar ? (
              ...
              <CircleUser
                aria-hidden='true'
                className='h-16 w-16 text-[var(--text)]'
              />
            )}
          ...
          <form onSubmit={handleSubmit} className='mt-6 space-y-5'>
            ...
            <div aria-live='polite' className='min-h-6'>
              {status && (
                <p
                  className={`text-sm ${status.state === 'error' ? 'text-red-600 dark:text-red-400' : 'text-green-700 dark:text-green-400'}`}
                >
                  {status.message}
                </p>
              )}
